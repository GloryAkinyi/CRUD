...
function updateUser($conn, $id, $u_name, $u_email)
{
    ...
    elseif (!filter_var($u_email, FILTER_VALIDATE_EMAIL)) {
        ...
        $check_email = mysqli_query($conn, "SELECT `email` FROM `glory` WHERE `email` = '$u_email' AND `id`!='$id'");
        ...
        // UPDATE USER DATA
        $query = mysqli_query($conn, "UPDATE `glory` SET `name`='$u_name', `email`='$u_email' WHERE `id`='$id'");
        // IF USER UPDATED
        if ($query) {
            return true;
        }
